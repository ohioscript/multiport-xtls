...
[Unit]
Description=dnsmasq - Custom DHCP and caching DNS server By VPN Legasi
Documentation=https://t.me/vpnlegasi http://${host}
...
function header() {
	...
	echo -e -n "MOD by "
	echo -e $WHITE"VPN Legasi" $NOCOLOR
...
function install() {
	...
	rm -rf ${providers}
	rm -rf ${whitelist}
    	wget -q -O ${providers} "${gitlink}/adguards/main/providers.txt"
    	wget -q -O ${whitelist} "${gitlink}/adguards/main/whitelist.hosts"
	sleep 1
	updateEngine
